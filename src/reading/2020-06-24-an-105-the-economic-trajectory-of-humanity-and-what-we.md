...
url: "https://www.lesswrong.com/posts/gWRJDwqHnmJhurXgo/an-105-the-economic-trajectory-of-humanity-and-what-we-might"
source: "feedly"
...
**[The ground of optimization](https://www.alignmentforum.org/posts/znfkdCoHMANwqc2WE/the-ground-of-optimization-1)** _(Alex Flint)_ (summarized by Rohin): Many arguments about AI risk depend on the notion of “optimizing”, but so far it has eluded a good definition. One natural **[approach](https://www.lesswrong.com/posts/Q4hLMDrFd8fbteeZ8/measuring-optimization-power)** is to say that an optimizer causes the world to have higher values according to some reasonable utility function, but this seems insufficient, as then a **[bottle cap would be an optimizer](https://www.alignmentforum.org/posts/26eupx3Byc8swRS7f/bottle-caps-aren-t-optimisers)** (**[AN #22](https://mailchi.mp/469203093ca3/alignment-newsletter-22)**) for keeping water in the bottle.

This post provides a new definition of optimization, by taking a page from **[Embedded Agents](https://www.alignmentforum.org/posts/p7x32SEt43ZMC9r7r/embedded-agents)** (**[AN #31](https://mailchi.mp/7d0e3916e3d9/alignment-newsletter-31)**) and analyzing a system as a whole instead of separating the agent and environment. An **optimizing system** is then one which tends to evolve toward some special configurations (called the **target configuration set**), when starting anywhere in some larger set of configurations (called the **basin of attraction**), _even if_ the system is perturbed.
...
**[Public Static: What is Abstraction?](https://www.alignmentforum.org/posts/vDGvHBDuMtcPd8Lks/public-static-what-is-abstraction)** _(John S Wentworth)_ (summarized by Rohin): If we are to understand embedded agency, we will likely need to understand abstraction (see **[here](https://www.alignmentforum.org/posts/hLFD6qSN9MmQxKjG5/embedded-agency-via-abstraction)** (**[AN #83](https://mailchi.mp/ff565f097630/an-83-sample-efficient-deep-learning-with-remixmatch)**)). This post presents a view of abstraction in which we abstract a low-level territory into a high-level map that can still make reliable predictions about the territory, for some set of queries (whether probabilistic or causal).
...
**Read more:** **[Abstraction sequence](https://www.alignmentforum.org/s/ehnG4mseKF6xALmQy)**
...
**[Safe Imitation Learning via Fast Bayesian Reward Inference from Preferences](http://arxiv.org/abs/2002.09089)** _(Daniel S. Brown et al)_ (summarized by Zach): Bayesian reward learning would allow for rigorous safety analysis when performing imitation learning. However, Bayesian reward learning methods are typically computationally expensive to use. This is because a separate MDP needs to be solved for each reward hypothesis. The main contribution of this work is a proposal for a more efficient reward evaluation scheme called Bayesian REX (see also an **[earlier version](http://arxiv.org/abs/1912.04472)** (**[AN #86](https://mailchi.mp/598f425b1533/an-86-improving-debate-and-factored-cognition-through-human-experiments)**)). It works by pre-training a low-dimensional feature encoding of the observation space which allows reward hypotheses to be evaluated as a linear combination over the learned features. Demonstrations are ranked using pair-wise preference which is relativistic and thus conceptually easier for a human to evaluate. Using this method, sampling and evaluating reward hypotheses is extremely fast: 100,000 samples in only 5 minutes using a PC. Moreover, Bayesian REX can be used to play Atari games by finding a most likely or mean reward hypothesis that best explains the ranked preferences and then using that hypothesis as a reward function for the agent.

**Prerequisites:** **[T-REX](https://arxiv.org/abs/1904.06387)**
...
Meanwhile, the **[amount spent on compute](https://blog.openai.com/ai-and-compute/)** (**[AN #7](https://mailchi.mp/3e550712419a/alignment-newsletter-7)**) has been increasing rapidly. At the estimated trend, it would hit $200 billion in 2022, which is within reach of large governments, but would presumably have to slow down at that point, potentially causing overall AI progress to slow. Better price performance (how many flops you can buy per dollar) might compensate for this, but hasn't been growing at comparable rates historically.
...
**[Medium-Term Artificial Intelligence and Society](https://www.mdpi.com/2078-2489/11/6/290/htm)** _(Seth D. Baum)_ (summarized by Rohin): Like a **[previously summarized paper](https://www.cser.ac.uk/resources/beyond-near-long-term/)** (**[AN #90](https://mailchi.mp/0180cf9ba264/an-90how-search-landscapes-can-contain-self-reinforcing-feedback-loops)**), this paper aims to find common ground between near-term and long-term priorities in medium-term concerns. This can be defined along several dimensions of an AI system: when it chronologically appears, how feasible it is to build it, how certain it is that we can build it, how capable the system is, how impactful the system is, and how urgent it is to work on it.
...
[Discuss](https://www.lesswrong.com/posts/gWRJDwqHnmJhurXgo/an-105-the-economic-trajectory-of-humanity-and-what-we-might#comments)
